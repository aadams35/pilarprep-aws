...
The shared request path is **React -> API Gateway -> Jobs API Lambda -> SQS -> AI Worker Lambda**. Briefs use Amazon Bedrock. Handoff, catch-up, and meeting analysis use AgentCore, with Strands coordinating the agent workflows. Scoped retrieval uses Bedrock Knowledge Bases and S3 Vectors. DynamoDB tracks application state; private S3 stores inputs, evidence, and generated documents.
