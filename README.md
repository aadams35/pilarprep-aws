...
The React app runs in the browser, represented by the Users icon. CloudFront serves the app from private S3 and proxies workspace API requests. The shared job pipeline is **API Gateway -> Jobs API Lambda -> SQS -> AI Worker Lambda**. Briefs use Amazon Bedrock. Handoff, catch-up, and meeting analysis use AgentCore, with Strands coordinating the agent workflows. Scoped retrieval uses Bedrock Knowledge Bases and S3 Vectors. DynamoDB tracks application state; private S3 stores inputs, evidence, and generated documents.
